...
Process job URLs stored in `data/pipeline.md`. The user adds URLs at any time and then executes `/applyd pipeline` to process them all.
...
```bash
node scripts/cv/cv-sync-check.mjs
```
