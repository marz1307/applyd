...
```bash
node scripts/metrics/followup-cadence.mjs
```
...
> "No active applications to follow up on. Apply to some roles first with `/applyd` and come back when they're aging."
...
- Suggest the user run `/applyd contacto {company}` to find the right person first
...
> - Trying a different contact via `/applyd contacto`
...
**To:** {email or "No contact found — run `/applyd contacto` first"}
...
These defaults can be overridden via `node scripts/metrics/followup-cadence.mjs --applied-days N`.
