...
// The six ATS hosts our own scanners hit were covered from day one, but the
// enterprise ATSs that large employers self-host on were not. Failure mode
// found in practice: a large-employer posting lives on a subdomain of an
// enterprise Avature tenant (e.g. `<company>consulting.avature.net`), and
// research took the ATS host as the EMPLOYER's site and probed /about,
// /about-us and /blog on it. Those soft-404'd, which the soft-404 detector
// caught — but had Avature served a real /about or an Impressum, the ATS
// vendor's own corporate copy and postal address would have shipped in the
// letter as the EMPLOYER'S. Same class of defect as a job board serving its
// own HQ address on multiple unrelated employers' postings. Enterprise ATSs
// belong on this list whether or not a scanner ever visits them.
// Workday/iCIMS/Taleo/SuccessFactors especially: they are the default for
// exactly the large DACH and UK employers in scope.
// pinpointhq.com added later when an existing tracked posting was repointed
// off a bot-walled Indeed URL onto a `<company>.pinpointhq.com` host. Every
// OTHER host in the
// pipeline was already covered - a sweep of all tracked job_urls found only
// one unmatched host, which turned out to be an employer's own careers
// subdomain (correctly not a portal). So this hole did not exist until the
// repoint was about to open it: research would have taken the Pinpoint ATS
// host for the employer's own site and probed it for facts and a postal
// address. Add the ATS host in the same change that starts pointing at it,
// never after.
