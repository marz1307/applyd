...
// ── Per-company failure ledger ──────────────────────────────────────
// Track consecutive failures per company so the routine output can
// surface stale portals (likely dead careers_url, renamed ATS slug,
// removed company). Schema:
//   { "<company-name>": { consecutive: int, last_error: str, last_seen_ok: ISO, last_attempt: ISO } }
function loadFailureLedger() {
  if (!existsSync(SCAN_FAILURES_PATH)) return {};
  try {
    return JSON.parse(readFileSync(SCAN_FAILURES_PATH, 'utf-8')) || {};
  } catch (e) {
    console.error(`⚠️  ${SCAN_FAILURES_PATH} unreadable (${e.message}) — starting fresh`);
    return {};
  }
}

function saveFailureLedger(ledger) {
  writeFileSync(SCAN_FAILURES_PATH, JSON.stringify(ledger, null, 2) + '\n', 'utf-8');
}

function updateFailureLedger(ledger, allCompanyNames, errorList, nowIso) {
  const errored = new Set(errorList.map(e => e.company));
  const errorByCompany = new Map(errorList.map(e => [e.company, e.error]));
  for (const name of allCompanyNames) {
    const prev = ledger[name] || { consecutive: 0, last_error: null, last_seen_ok: null, last_attempt: null };
    if (errored.has(name)) {
      ledger[name] = {
        consecutive: (prev.consecutive || 0) + 1,
        last_error: errorByCompany.get(name) || 'unknown',
        last_seen_ok: prev.last_seen_ok,
        last_attempt: nowIso,
      };
    } else {
      ledger[name] = {
...
function staleCompanies(ledger) {
  return Object.entries(ledger)
    .filter(([_, v]) => (v.consecutive || 0) >= STALE_THRESHOLD)
...
async function main() {
  ...
  // ── Failure ledger update + stale-company report ─────────────────
  // `targets` is the full list of companies we attempted this run.
  // Companies absent from `errors` are treated as healthy and their
  // consecutive counter resets to 0.
  let stale = [];
  if (!dryRun) {
    const ledger = loadFailureLedger();
    const allCompanyNames = targets.map(t => t.name);
    updateFailureLedger(ledger, allCompanyNames, errors, new Date().toISOString());
    saveFailureLedger(ledger);
    stale = staleCompanies(ledger);
    if (stale.length > 0) {
      console.log(`\nStale portals (≥${STALE_THRESHOLD} consecutive failures, likely dead URLs):`);
      for (const s of stale) {
        console.log(`  ⚠ ${s.name}: ${s.consecutive} consec failures — last error: ${s.last_error}`);
      }
    }
  }

  // ── Machine-stable summary block ─────────────────────────────────
  // Routine prompts must echo these values verbatim into ROUTINE_CONTRACT
  // rather than constructing them by hand. Anything downstream that
  // parses scan output should read between the SCAN_CONTRACT markers.
  console.log("");
  console.log("--- SCAN_CONTRACT ---");
  console.log(`SCRIPT: scan.mjs`);
  console.log(`TIMESTAMP_UTC: ${new Date().toISOString()}`);
  console.log(`PORTALS_SCANNED: ${targets.length}`);
  console.log(`HITS_RAW: ${totalFound}`);
  console.log(`HITS_AFTER_FILTER: ${verifiedOffers.length}`);
  console.log(`DUPLICATES_SKIPPED: ${totalDupes}`);
  console.log(`ERRORS: ${errors.length}`);
  console.log(`STALE_COMPANIES_COUNT: ${stale.length}`);
